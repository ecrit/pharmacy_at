...
public class PrescriptionDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(Composite parent){
		...
		setMessage("Detailed prescription information", IMessageProvider.INFORMATION);
		...
		if (!selArticles.isEmpty()) {
			setCheckedArticles();
		}
		return area;
	}
	
	private void setCheckedArticles(){
		for (TableItem item : tableViewer.getTable().getItems()) {
			if (selArticles.contains(item.getData())) {
	...
	private void createIssuingPractitionerPart(Composite container){
		...
		txtIssuingPractitioner.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
	}
...
// setCheckedArticles();
	...
	protected DataBindingContext initDataBinding(){
		...
		//
		IObservableValue numberObserveValue =
			EMFProperties.value(ModelPackage.Literals.PRESCRIPTION__NUMBER).observe(p);
		IObservableValue textTxtNumberObserveValue =
			WidgetProperties.text(SWT.Modify).observe(txtNumber);
		...
		Binding bindValue =
			bindingContext.bindValue(textTxtNumberObserveValue, numberObserveValue, stringToInt,
				intToString);
		ControlDecorationSupport.create(bindValue, SWT.TOP | SWT.LEFT);
		//
		IObservableValue practitionerObserveValue =
			EMFProperties.value(ModelPackage.Literals.PRESCRIPTION__ISSUING_PRACTITIONER)
				.observe(p);
		IObservableValue textTxtIssuingPractitionerObserveValue =
			WidgetProperties.text(SWT.Modify).observe(txtIssuingPractitioner);
