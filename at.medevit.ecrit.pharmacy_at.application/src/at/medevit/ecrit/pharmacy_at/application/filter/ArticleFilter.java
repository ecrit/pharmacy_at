...
public class ArticleFilter extends ViewerFilter {
	...
	public boolean select(Viewer viewer, Object parentElement, Object element){
		...
		if ((Integer.toString(article.getAdmissionNumber())).contains(searchTxt)) {
			return true;
		}
		
		if (article.getAvailability() != null
			&& article.getAvailability().toString().toLowerCase().contains(searchTxt)) {
			return true;
		}
		
		if (article.getName() != null && (article.getName().toLowerCase()).contains(searchTxt)) {
			return true;
		}
		
		if (article.getDescription() != null
			&& (article.getDescription().toLowerCase()).contains(searchTxt)) {
			return true;
		}
