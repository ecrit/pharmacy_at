...
import at.medevit.ecrit.pharmacy_at.application.part.BillPart;
...
public class AddToInvoiceHandler {
	...
	public void execute(@Named(IServiceConstants.ACTIVE_SHELL) Shell shell) {
		StockArticle a = (StockArticle) selectionService.getSelection(ID_ARTICLE_PART);
		a.setNumberOnStock(a.getNumberOnStock() - 1);
		
		SampleModel.getInvoice().getArticle().add(a.getArticle());
		
		MPart part = partService.findPart(ID_BILL_PART);
		BillPart billPart = (BillPart) part.getObject();
		billPart.addArticleAndUpdate(a.getArticle());
	}
